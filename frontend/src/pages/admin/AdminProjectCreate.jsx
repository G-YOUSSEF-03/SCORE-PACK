...
function AdminProjectCreate() {
  ...
  const updateField = (event) => {
    ...
    setForm((current) => {
      ...
      if (name === 'title') {
        ...
        if (!current.meta_title) next.meta_title = value ? `${value} | SCORE PACK` : ''
      }
      return next
    })
  ...
  const submit = async (isPublished) => {
    const clientErrors = validateForm(form, image)
    ...
    try {
      ...
      notify(isPublished ? 'Projet publie avec succes.' : 'Projet enregistre en brouillon.')
      setForm(initialForm)
      setImage(null)
      if (preview) URL.revokeObjectURL(preview)
      setPreview('')
      ...
      notify(apiErrorMessage(error, 'Impossible d enregistrer le projet.'), 'error')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1480px] space-y-6">
      <section>
        <h1 className="text-[34px] font-extrabold leading-tight tracking-[-0.04em] text-[#071f49]">Ajouter un projet</h1>
        <nav className="mt-4 flex items-center gap-3 text-[15px] font-medium text-[#33496f]" aria-label="Fil d'Ariane">
          ...
          <ChevronRight size={17} className="text-[#8a9ab5]" />
          <span className="text-[#c88a22]">Ajouter un projet</span>
        </nav>
      </section>

      <section className="grid gap-5 xl:grid-cols-[1.03fr_1fr]">
        <Panel title="1. Informations generales" icon={Clipboard}>
          <Field label="Titre du projet" name="title" value={form.title} onChange={updateField} error={errors.title} required placeholder="Ex: Complexe residentiel a Casablanca" />
          <div className="mt-5 grid gap-4 md:grid-cols-[1fr_0.9fr_0.85fr]">
            ...
            <Field label="Localisation / Ville" name="location" value={form.location} onChange={updateField} error={errors.location} required placeholder="Ex: Casablanca" />
        ...
        <Panel title="2. Image du projet" icon={ImageIcon}>
          <div className="grid gap-5 md:grid-cols-[1fr_0.82fr]">
            <button type="button" onClick={() => fileInputRef.current?.click()} className="min-h-[220px] rounded-[14px] border-2 border-dashed border-[#d7e1ee] bg-white text-center transition hover:border-[#c88a22]">
            ...
            <div>
              <p className="mb-3 text-sm font-extrabold text-[#071f49]">Apercu</p>
              <div className="grid min-h-[190px] place-items-center overflow-hidden rounded-[9px] bg-[#edf1f6]">
                {preview ? <img src={preview} alt="" className="h-full min-h-[190px] w-full object-cover" /> : <ImageIcon size={72} className="text-[#cbd3df]" fill="currentColor" strokeWidth={1.4} />}
              </div>
            </div>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.03fr_1fr]">
        <Panel title="3. Description" icon={FileText}>
          <TextArea label="Description courte" name="short_description" value={form.short_description} onChange={updateField} error={errors.short_description} required placeholder="Ex: Description courte du projet (150 caracteres max)" rows={2} />
          <label className="mt-5 block">
            ...
            <div className="mt-2 overflow-hidden rounded-[9px] border border-[#dce4ef] bg-white">
              ...
              <textarea name="detailed_description" value={form.detailed_description} onChange={updateField} className="min-h-[96px] w-full resize-none border-0 bg-white px-4 py-3 text-[#071f49] outline-none" />
            </div>
        ...
        <Panel title="4. Details du projet" icon={Clipboard}>
          <div className="grid gap-5 md:grid-cols-2">
            <Field label="Client / Promoteur" name="client" value={form.client} onChange={updateField} error={errors.client} placeholder="Ex: Societe Immobiliere XYZ" />
            ...
            <Field label="Budget estime" name="estimated_budget" value={form.estimated_budget} onChange={updateField} error={errors.estimated_budget} placeholder="Ex: 15 000 000 MAD" />
            <Field label="Duree de realisation" name="duration" value={form.duration} onChange={updateField} error={errors.duration} placeholder="Ex: 24 mois" />
          </div>
        </Panel>
      </section>
      ...
      <Panel title="5. SEO (Referencement)" icon={FileText}>
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1fr_1.3fr]">
          <Field label="Slug" name="slug" value={form.slug} onChange={updateField} error={errors.slug} required placeholder="ex: complexe-residentiel-casablanca" help="Genere automatiquement a partir du titre" />
          <Field label="Meta title" name="meta_title" value={form.meta_title} onChange={updateField} error={errors.meta_title} placeholder="Ex: Complexe residentiel a Casablanca | SCORE PACK" help="Recommande: 50-60 caracteres" />
          <TextArea label="Meta description" name="meta_description" value={form.meta_description} onChange={updateField} error={errors.meta_description} placeholder="Ex: Decouvrez notre projet de complexe residentiel a Casablanca..." rows={2} help="Recommande: 150-160 caracteres" />
        </div>
      </Panel>
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button type="button" disabled={submitting} onClick={() => submit(true)} className="inline-flex h-12 items-center gap-5 rounded-[8px] bg-[#061f49] px-8 text-sm font-extrabold text-white shadow-[0_14px_28px_rgba(6,31,73,0.18)] transition hover:bg-[#0b2d63] disabled:opacity-60">
            {submitting ? 'Enregistrement...' : 'Publier le projet'}
            <Send size={17} />
          </button>
        </div>
      </section>
...
function validateForm(form, image) {
  ...
  if (!image) errors.project_image = 'L image du projet est obligatoire.'
  return errors
}
