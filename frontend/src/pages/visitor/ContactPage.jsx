...
const companyAddress =
  "RUE SARIA BEN ZOUNAIM ETG 3 APPT 3, PALMIER, CASABLANCA, MOROCCO";
...
function ContactPage({ publicData }) {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        message: apiErrorMessage(error, "Impossible d’envoyer votre message."),
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-7 text-[#061f49]">
      <section className="grid items-center gap-10 rounded-[26px] bg-[#f7f9fc] py-5 lg:grid-cols-[0.92fr_1.08fr]">
        <div>
          <nav
            className="flex items-center gap-2 text-[13px] font-bold text-[#6f7d91]"
            aria-label="Fil d’Ariane"
          >
            <a href="#accueil" className="transition hover:text-[#b8842c]">
              Accueil
            </a>
          ...
          <p className="mt-7 max-w-[640px] text-[17px] leading-8 text-[#43526a]">
            ...
            et vous accompagner dans vos projets d’investissement.
          </p>
        </div>
      ...
      <Reveal className="grid gap-6 lg:grid-cols-[0.96fr_1.04fr]">
        <Reveal
          ...
          delay={0.1}
          onSubmit={onSubmit}
          className="rounded-[22px] border border-[#e6ebf2] bg-white p-6 shadow-[0_18px_50px_rgba(6,31,73,0.08)] sm:p-8"
        >
          <h2 className="text-[28px] font-extrabold tracking-[-0.025em] text-[#061f49]">
          ...
          <label className="mt-5 block">
            <span className="text-sm font-extrabold text-[#061f49]">
              Votre message *
            </span>
            <textarea
              name="message"
              value={form.message}
              onChange={onChange}
              className="mt-2 min-h-[154px] w-full resize-none rounded-[13px] border border-[#dfe6ef] bg-[#f7f9fc] px-4 py-3 text-[#061f49] outline-none transition focus:border-[#b8842c] focus:bg-white"
            />
            {errors.message ? (
          ...
          <button
            type="submit"
            disabled={submitting}
            className="mt-6 inline-flex h-14 items-center gap-5 rounded-[10px] bg-[#061f49] px-7 text-sm font-extrabold text-white shadow-[0_16px_32px_rgba(6,31,73,0.2)] transition hover:bg-[#0d2d62] disabled:opacity-60"
          >
      ...
      <Reveal className="rounded-[24px] bg-[#f7f9fc] py-7">
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="text-[38px] font-extrabold leading-tight tracking-[-0.035em] text-[#061f49]">
...
function ContactInfoCard({ icon: Icon, title, lines }) {
  return (
    <article className="min-h-[178px] rounded-[20px] border border-[#e6ebf2] bg-white p-6 shadow-[0_18px_48px_rgba(6,31,73,0.08)]">
      ...
      <h2 className="mt-5 text-[18px] font-extrabold tracking-[-0.015em] text-[#061f49]">
        {title}
      </h2>
...
function buildContactCards(settings) {
  ...
  const hours = String(settings.working_hours || fallbackSettings.working_hours)
    .split("/")
    .map((line) => line.trim());
  ...
  return [
    {
      ...
      lines: [
        ...
        settings.tagline || fallbackSettings.tagline,
      ],
    },
    {
  ...
  return (
    <div className="relative min-h-[520px] overflow-hidden rounded-[22px] border border-[#e6ebf2] bg-[#dfe7df] shadow-[0_18px_50px_rgba(6,31,73,0.08)]">
      ...
      <div className="absolute left-1/2 top-[55%] z-20 w-[260px] -translate-x-1/2 rounded-[16px] bg-white p-4 shadow-[0_18px_44px_rgba(6,31,73,0.18)]">
        <p className="text-[15px] font-extrabold text-[#061f49]">
          {settings.company_name}
        </p>
        <p className="mt-2 text-[13px] font-semibold leading-5 text-[#53647c]">
          {companyAddress}
        </p>
      </div>
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="absolute left-5 top-5 rounded-[10px] bg-white px-4 py-2 text-xs font-extrabold text-[#061f49] shadow-[0_10px_24px_rgba(6,31,73,0.12)] transition hover:text-[#b8842c]"
      >
        Get Directions
      </a>
    </div>
...
function ReasonCard({ icon: Icon, title, text }) {
  return (
    <article className="min-h-[238px] rounded-[22px] border border-[#e6ebf2] bg-white p-7 text-center shadow-[0_18px_48px_rgba(6,31,73,0.08)]">
      ...
      <h3 className="mt-6 text-xl font-extrabold tracking-[-0.02em] text-[#061f49]">
        {title}
      </h3>
  ...
  return (
    <Reveal className="pb-1">
      <div className="relative overflow-hidden rounded-[26px] bg-[#061f49] px-7 py-8 text-white shadow-[0_24px_60px_rgba(6,31,73,0.22)] sm:px-10 lg:flex lg:min-h-[176px] lg:items-center lg:justify-between">
        ...
        <div className="relative">
          <h2 className="text-[31px] font-extrabold tracking-[-0.035em]">
        ...
        <a
          href="#devis"
          className="relative mt-7 inline-flex h-14 items-center gap-7 rounded-[10px] bg-[#b8842c] px-8 text-sm font-extrabold text-white shadow-[0_14px_30px_rgba(184,132,44,0.28)] lg:mt-0"
        >
          Demander un devis
          <ArrowRight size={18} />
        </a>
