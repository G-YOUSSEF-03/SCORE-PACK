...
const stats = [
  {
    ...
    text: "Dans divers secteurs d'activité",
  },
  {
    icon: Shield,
    number: "98%",
    title: "Taux de satisfaction",
    text: "La réussite de nos clients est notre priorité",
  },
  {
    icon: BriefcaseBusiness,
    number: "10+",
    title: "Années d'expérience",
    text: "Une expertise reconnue dans les études de projets",
  },
  {
    icon: Handshake,
    number: "50+",
    title: "Partenaires & Investisseurs",
    text: "Nous collaborons avec un large réseau de partenaires",
  },
];
...
const values = [
  {
    icon: Shield,
    title: "Intégrité",
    text: "Nous agissons avec transparence, honnêteté et responsabilité dans toutes nos relations.",
  },
  {
    icon: Medal,
    title: "Excellence",
    text: "Nous visons l'excellence à chaque étape pour garantir des résultats fiables et durables.",
  },
  {
    icon: Target,
    title: "Engagement",
    text: "Nous nous engageons pleinement pour la réussite de vos projets, comme si c'étaient les nôtres.",
  },
  {
    icon: Lightbulb,
    title: "Innovation",
    text: "Nous adoptons des approches modernes et innovantes pour répondre aux défis d'aujourd'hui.",
  },
  {
    icon: Users,
    title: "Collaboration",
    text: "Nous croyons au pouvoir du partenariat pour créer une valeur partagée et durable.",
  },
];
...
function usePublicSiteData() {
  ...
  useEffect(() => {
    ...
    publicApi
      ...
      .then((response) => {
        if (!ignore)
          setServicesData(
            normalizeApiRows(response).filter(
    ...
    publicApi
      ...
      .then((response) => {
        if (!ignore && response)
          setSettingsData({ ...defaultSettings, ...response });
      })
      .catch((error) => {
        if (!ignore)
          setErrors((current) => ({
            ...current,
            settings: apiErrorMessage(
              error,
  ...
  const mappedProjects = useMemo(
    () => projectsData.map(mapApiProject),
    [projectsData],
  );
...
function VisitorSite({ serviceDetails = false }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] text-[#0F2747]">
      <Header
        activeHash={serviceDetails ? "#services" : activeHash}
        linkPrefix={serviceDetails ? "/" : ""}
      />
...
function getCurrentHash() {
  ...
  return routes.some((route) => route.hash === hash) || hash === "#devis"
    ? hash
    : "#accueil";
}
    ...
    <header className="fixed inset-x-0 top-0 z-50 px-5 pt-3 sm:px-8">
      <div className="mx-auto max-w-[1470px] rounded-[16px] border border-white/80 bg-white/95 px-5 shadow-[0_16px_42px_rgba(15,39,71,0.1)] backdrop-blur xl:px-10">
        <div className="flex h-[76px] items-center justify-between">
              ...
              <a
                ...
                className={
                  activeHash === route.hash
                    ? "nav-active text-[#A77A33]"
                    : "transition hover:text-[#A77A33]"
                }
              >
                {route.label}
              </a>
          ...
          <a
            href={`${linkPrefix}#devis`}
            className="hidden h-12 items-center gap-5 rounded-[8px] bg-[#0F2747] px-6 text-sm font-extrabold text-white shadow-[0_12px_28px_rgba(15,39,71,0.18)] md:inline-flex"
          >
            Demander un devis
            <ArrowRight size={18} />
          </a>
          ...
          <button
            className="grid size-11 place-items-center rounded-[9px] border border-[#E2E7EE] lg:hidden"
            aria-label="Menu"
...
function HomePage({ publicData }) {
  ...
  return (
    <>
      ...
      >
        <div>
          ...
          <h1 className="mt-4 max-w-[650px] text-[46px] font-extrabold leading-[1.03] tracking-[-0.045em] text-[#0F2747] sm:text-[64px]">
            Bureau d&apos;études des projets{" "}
            <span className="text-[#A77A33]">d&apos;investissement</span>
          </h1>
          <p className="mt-7 max-w-[590px] text-[17px] leading-8 text-[#2F405A]">
            SCORE PACK accompagne les porteurs de projets, investisseurs et
            entreprises avec des études fiables, des analyses solides et des
            solutions sur mesure.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            <a
              href="#services"
              className="inline-flex h-14 items-center gap-4 rounded-[10px] bg-[#0F2747] px-7 text-sm font-extrabold text-white shadow-[0_16px_30px_rgba(15,39,71,0.2)]"
            >
            ...
            <a
              href="#a-propos"
              className="inline-flex h-14 items-center gap-4 rounded-[10px] border border-[#D9E0E8] bg-white px-7 text-sm font-extrabold text-[#0F2747] shadow-sm"
            >
              À propos de nous
      ...
      <ResourceState
        loading={publicData.loading.services}
        error={publicData.errors.services}
        empty={!homeServices.length}
        emptyText="Aucun service actif disponible pour le moment."
      />
      ...
      <ResourceState
        loading={publicData.loading.projects}
        error={publicData.errors.projects}
        empty={!homeProjects.length}
        emptyText="Aucun projet publié disponible pour le moment."
      />
...
function AboutPage() {
  return (
    <>
      <section
        id="a-propos"
        className="grid items-center gap-14 lg:grid-cols-[0.88fr_1.32fr]"
      >
        <div className="pl-0 lg:pl-1">
          ...
          <p className="mt-7 max-w-[575px] text-[16px] leading-8 text-[#2F405A]">
            SCORE PACK est un bureau d&apos;études spécialisé dans
            l&apos;accompagnement des porteurs de projets et des investisseurs
            dans la réussite de leurs projets d&apos;investissement. Nous
            transformons vos idées en projets viables, durables et rentables.
          </p>
        </div>
      ...
      <Reveal className="mt-6 grid gap-5 lg:grid-cols-[1.08fr_0.95fr_1.08fr]">
        <Reveal
          as="div"
          delay={0.1}
          className="rounded-[15px] border border-[#E5EAF0] bg-white p-8 shadow-[0_16px_44px_rgba(15,39,71,0.07)]"
        >
          <InfoBlock
            icon={Target}
            label="Notre mission"
        ...
        >
          ...
          <ul className="mt-6 space-y-5">
            {expertise.map((item) => (
              <li
                key={item}
                className="flex items-start gap-4 text-[15px] leading-6 text-[#263A58]"
              >
                <CheckCircle2
                  size={19}
                  className="mt-0.5 shrink-0 text-[#C79A4A]"
                />
                <span>{item}</span>
              </li>
            ))}
  ...
  return (
    <>
      ...
      >
        <div>
          ...
          <p className="mt-7 max-w-[560px] text-[17px] leading-8 text-[#2F405A]">
            Des solutions sur mesure pour accompagner vos projets à chaque
            étape, avec rigueur, expertise et engagement.
          </p>
        </div>
      ...
      <ResourceState
        loading={publicData.loading.services}
        error={publicData.errors.services}
        empty={!publicData.services.length}
        emptyText="Aucun service actif disponible pour le moment."
      />
...
function ProjectsPage({ publicData }) {
  ...
  const dynamicCategories = useMemo(
    () => [
      "Tous",
      ...Array.from(
        new Set(projectRows.map((project) => project.category).filter(Boolean)),
      ),
    ],
    [projectRows],
  );
  ...
  return (
    <PageShell
      eyebrow="Projets"
      title="Des réalisations qui parlent d'elles-mêmes"
    >
      <Reveal as="div" className="grid gap-6 lg:grid-cols-[280px_1fr]">
        <Reveal
          as="aside"
          delay={0.1}
          className="h-fit rounded-[18px] border border-[#E5EAF0] bg-white p-6 shadow-[0_16px_44px_rgba(15,39,71,0.07)]"
        >
          <div className="flex items-center justify-between gap-4">
            <h2 className="text-lg font-extrabold tracking-[-0.02em] text-[#0F2747]">
              Filtres
            </h2>
            <button
              type="button"
              onClick={resetFilters}
              className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#A77A33]"
            >
              Réinitialiser
            </button>
          </div>
          ...
          <div className="mt-6">
            <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#6D7890]">
              Catégories
            </p>
        ...
        <Reveal delay={0.2}>
          <div className="rounded-[18px] border border-[#E5EAF0] bg-white p-5 shadow-[0_16px_44px_rgba(15,39,71,0.07)]">
            <div className="grid gap-4 xl:grid-cols-[1fr_210px_auto] xl:items-center">
              <label className="relative block">
                <Search
                  size={19}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-[#A77A33]"
                />
                <input
                  value={searchTerm}
                  onChange={(event) => {
          ...
          {visibleProjects.length === 0 ? (
            <div className="mt-5 rounded-[18px] border border-[#E5EAF0] bg-white p-10 text-center shadow-[0_16px_44px_rgba(15,39,71,0.07)]">
              <p className="text-lg font-extrabold text-[#0F2747]">
                Aucun projet ne correspond à votre recherche.
              </p>
              <button
                type="button"
                onClick={resetFilters}
                className="mt-4 text-sm font-extrabold text-[#A77A33]"
              >
      ...
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-[16px] border border-[#E5EAF0] bg-white p-8 shadow-[0_16px_44px_rgba(15,39,71,0.07)]">
          ...
          <p className="mt-4 text-[15px] leading-7 text-[#3D4E66]">
            Envoyez-nous les premières informations de votre projet. Notre
            équipe reviendra vers vous avec une réponse adaptée.
          </p>
          <div className="mt-6 space-y-5 text-[15px] font-semibold text-[#263A58]">
            ...
            <a
              href={`mailto:${publicDisplayEmail}`}
        ...
        <form
          onSubmit={onSubmit}
          className="rounded-[16px] border border-[#E5EAF0] bg-white p-8 shadow-[0_16px_44px_rgba(15,39,71,0.07)]"
        >
          ...
          <label className="mt-4 block">
            <span className="text-sm font-extrabold text-[#0F2747]">
              Message
            </span>
            <textarea
              name="message"
              value={form.message}
              onChange={onChange}
              className="mt-2 min-h-32 w-full rounded-[10px] border border-[#DDE4EC] bg-[#F8F9FB] px-4 py-3 outline-none focus:border-[#C79A4A]"
            />
            {errors.message ? (
          ...
          {status.message ? (
            <p
              className={`mt-4 text-sm font-bold ${status.type === "success" ? "text-emerald-700" : "text-red-600"}`}
            >
              {status.message}
            </p>
          ) : null}
          <button
            disabled={submitting}
            type="submit"
            className="mt-5 inline-flex h-12 items-center gap-4 rounded-[8px] bg-[#0F2747] px-6 text-sm font-extrabold text-white disabled:opacity-60"
          >
...
function validateQuoteForm(form) {
  ...
  if (!form.project_type.trim())
    errors.project_type = "Le type de projet est obligatoire.";
  if (!form.project_title.trim())
    errors.project_title = "Le titre du projet est obligatoire.";
    ...
    >
      <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <form className="rounded-[16px] border border-[#E5EAF0] bg-white p-8 shadow-[0_16px_44px_rgba(15,39,71,0.07)]">
          ...
          <button
            type="button"
            className="mt-5 inline-flex h-12 items-center gap-4 rounded-[8px] bg-[#0F2747] px-6 text-sm font-extrabold text-white"
          >
            Envoyer
            <ArrowRight size={18} />
          </button>
  ...
  return (
    <section>
      <div className="mb-8">
        ...
        <h1 className="mt-3 max-w-[850px] text-[42px] font-extrabold leading-[1.05] tracking-[-0.045em] sm:text-[58px]">
          {title}
        </h1>
      </div>
      {children}
    ...
    <Reveal className="mt-6">
      <div className="relative overflow-hidden rounded-[18px] bg-[#0F2747] px-8 py-7 text-white shadow-[0_22px_55px_rgba(15,39,71,0.2)] sm:px-10 lg:flex lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-[30px] font-extrabold tracking-[-0.035em]">
        ...
        <a
          href="#devis"
          className="mt-7 inline-flex h-14 items-center gap-9 rounded-[8px] bg-[#C79A4A] px-9 text-sm font-extrabold text-white shadow-[0_14px_30px_rgba(199,154,74,0.26)] lg:mt-0"
        >
          Demander un devis
          <ArrowRight size={19} />
        </a>
...
function StatCard({ icon: Icon, number, title, text, isLast }) {
  return (
    <article
      className={`flex items-center gap-6 px-9 py-7 ${isLast ? "" : "lg:border-r lg:border-[#DDE4EC]"}`}
    >
      <span className="grid size-16 shrink-0 place-items-center rounded-full bg-[#F4F5F7] text-[#0F2747]">
        <Icon size={31} strokeWidth={2.6} />
      </span>
      <div>
        <p className="text-[37px] font-extrabold leading-none tracking-[-0.045em] text-[#0F2747]">
          {number}
        </p>
        <h3 className="mt-2 text-sm font-extrabold text-[#0F2747]">{title}</h3>
        <p className="mt-2 max-w-[210px] text-[13px] leading-5 text-[#3D4E66]">
          {text}
        </p>
      </div>
    </article>
...
function SectionLabel({ children }) {
  return (
    <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#A77A33]">
      {children}
    </p>
  );
}
...
function ServiceCard({ icon: Icon, title, text, slug }) {
  ...
  return (
    <article className="rounded-[16px] border border-[#E5EAF0] bg-white p-7 shadow-[0_16px_44px_rgba(15,39,71,0.07)]">
      ...
      <h2 className="mt-5 text-lg font-extrabold tracking-[-0.02em]">
        {title}
      </h2>
      <p className="mt-3 text-sm leading-6 text-[#3D4E66]">{text}</p>
      <a
        href={detailsHref}
        className="mt-5 inline-flex items-center gap-3 text-sm font-extrabold text-[#A77A33]"
      >
        En savoir plus
        <ArrowRight size={16} />
      </a>
...
function ServiceDetailCard({
  ...
  return (
    <article className="flex min-h-[390px] flex-col rounded-[16px] border border-[#E5EAF0] bg-white p-7 shadow-[0_18px_48px_rgba(15,39,71,0.08)]">
      ...
      {bullets.length ? (
        <ul className="mt-7 space-y-4">
          {bullets.map((bullet) => (
            <li
              key={bullet}
              className="flex items-start gap-3 text-sm leading-5 text-[#263A58]"
            >
              <CheckCircle2
                size={16}
                className="mt-0.5 shrink-0 text-[#A77A33]"
              />
              <span>{bullet}</span>
            </li>
          ))}
      ...
      <a
        href={detailsHref}
        className="mt-auto inline-flex items-center gap-4 pt-8 text-sm font-extrabold text-[#A77A33]"
      >
        En savoir plus
        <ArrowRight size={17} />
      </a>
...
function ApproachStep({ icon: Icon, index, title, text }) {
  return (
    <div className="relative text-center">
      ...
      <h3 className="mt-6 text-sm font-extrabold text-[#0F2747]">
        {index}. {title}
      </h3>
      <p className="mx-auto mt-3 max-w-[190px] text-sm leading-6 text-[#3D4E66]">
        {text}
      </p>
    </div>
  );
    ...
    <button
      ...
      className={`grid size-10 place-items-center rounded-[8px] transition ${
        active
          ...
          : "text-[#6D7890] hover:bg-white hover:text-[#0F2747]"
      }`}
    >
      {children}
    </button>
      ...
      <article className="grid overflow-hidden rounded-[18px] border border-[#E5EAF0] bg-white shadow-[0_16px_44px_rgba(15,39,71,0.07)] md:grid-cols-[260px_1fr]">
        ...
        <div className="flex flex-col p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl font-extrabold tracking-[-0.03em] text-[#0F2747]">
                {name}
              </h2>
              <p className="mt-2 text-sm font-semibold text-[#5A6B82]">
                {city} · {year}
              </p>
            </div>
            <span className="rounded-full bg-[#F4F1ED] px-4 py-2 text-sm font-extrabold text-[#A77A33]">
              {investment}
            </span>
          </div>
          <p className="mt-5 max-w-3xl text-sm leading-6 text-[#3D4E66]">
            {description}
          </p>
          <a
            href="#contact"
            className="mt-auto inline-flex items-center gap-3 pt-6 text-sm font-extrabold text-[#A77A33]"
          >
            Voir le projet
            <ArrowRight size={17} />
          </a>
  ...
  return (
    <article className="overflow-hidden rounded-[18px] border border-[#E5EAF0] bg-white shadow-[0_16px_44px_rgba(15,39,71,0.07)] transition hover:-translate-y-1 hover:shadow-[0_22px_54px_rgba(15,39,71,0.11)]">
      ...
      <div className="p-5">
        ...
        <h2 className="mt-3 text-lg font-extrabold leading-tight tracking-[-0.02em] text-[#0F2747]">
          {name}
        </h2>
        <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#3D4E66]">
          {description}
        </p>
        <div className="mt-5 flex items-center justify-between gap-3 border-t border-[#E8ECF1] pt-4">
          ...
          <a
            href="#contact"
            className="inline-flex items-center gap-2 text-sm font-extrabold text-[#A77A33]"
          >
            Détails
            <ArrowRight size={16} />
          </a>
...
function ResourceState({ loading, error, empty, emptyText }) {
  if (loading) {
    return (
      <p className="mt-4 text-sm font-bold text-[#5A6B82]">
...
function Input({ label, className = "", error, ...props }) {
  return (
    <label className={`block ${className}`}>
      ...
      <input
        className="mt-2 h-12 w-full rounded-[10px] border border-[#DDE4EC] bg-[#F8F9FB] px-4 outline-none focus:border-[#C79A4A]"
        {...props}
      />
      {error ? (
...
function Footer({ settings = defaultSettings, linkPrefix = "" }) {
  const address =
    [settings.address, settings.city, settings.country]
      .filter(Boolean)
      .join(", ") || "Casablanca, Maroc";
  const withPrefix = (href) =>
    href.startsWith("#") ? `${linkPrefix}${href}` : href;
  ...
  return (
    <footer className="mt-0 bg-white">
      <div className="mx-auto max-w-[1400px] px-5 py-8 sm:px-8">
        <Reveal
          as="div"
          className="grid gap-9 md:grid-cols-2 lg:grid-cols-[1.25fr_0.8fr_1fr_0.9fr_1.25fr_0.85fr]"
        >
          <div>
            <a
              href={withPrefix("#accueil")}
              className="flex items-center gap-3"
            >
          ...
          <FooterColumn
            ...
            linkPrefix={linkPrefix}
            links={[
              ["Études de faisabilité", "#services"],
              ["Études techniques", "#services"],
              ["Études financières", "#services"],
              ["Montage de dossiers", "#services"],
            ]}
          />
          <FooterColumn
            title="Liens rapides"
          ...
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <a
                href={settings.linkedin_url || withPrefix("#accueil")}
                aria-label="LinkedIn"
                className="grid size-9 place-items-center rounded-full bg-[#0F2747] text-xs font-extrabold text-white"
              >
                in
              </a>
              <a
                href={settings.facebook_url || withPrefix("#accueil")}
                aria-label="Facebook"
                className="grid size-9 place-items-center rounded-full bg-[#0F2747] text-sm font-extrabold text-white"
              >
                f
              </a>
              <a
                href={settings.instagram_url || withPrefix("#accueil")}
              ...
              <a
                href={settings.youtube_url || withPrefix("#accueil")}
                aria-label="YouTube"
                className="grid size-9 place-items-center rounded-full bg-[#0F2747] text-xs font-extrabold text-white"
              >
                ▶
        ...
        <p className="mt-8 text-center text-sm text-[#5A6B82]">
          © 2026 {settings.company_name || defaultSettings.company_name}. Tous
          droits réservés.
        </p>
      </div>
    </footer>
    ...
    <div>
      ...
      <ul className="mt-5 space-y-3 text-sm font-semibold text-[#3D4E66]">
        {links.map(([label, href]) => (
          <li key={label}>
            <a href={withPrefix(href)} className="hover:text-[#A77A33]">
              {label}
            </a>
          </li>
        ))}
